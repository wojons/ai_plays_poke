...
class TestVisionClientParsing:
    ...
    def test_clean_json_strips_markdown_fences(self) -> None:
        text = '```json\n{"screen_type": "battle"}\n```'
        cleaned = VisionClient._clean_json_text(text)
        assert cleaned == '{"screen_type": "battle"}'

    def test_clean_json_strips_bare_fences(self) -> None:
        text = '```\n{"screen_type": "overworld"}\n```'
        cleaned = VisionClient._clean_json_text(text)
    ...
    def test_regex_extract_screen_type(self) -> None:
    ...
    def test_regex_extract_text_lines(self) -> None:
    ...
    def test_regex_extract_empty(self) -> None:
...
class TestVisionClientEncoding:
    ...
    def test_encode_image_produces_valid_png_base64(self) -> None:
    ...
    def test_compute_hash_same_image(self) -> None:
        img = _make_rgb_array()
    ...
    def test_compute_hash_is_hex_string(self) -> None:
        img = _make_rgb_array()
...
class TestPromptStackCore:
    ...
    def test_assemble_with_menu_options_fallback(self) -> None:
    ...
    def test_assemble_with_minimal_input(self) -> None:
    ...
    def test_available_stacks_with_missing_dir(self) -> None:
...
class TestPromptStackHelpers:
    ...
    def test_safe_dict_existing_key(self) -> None:
        d = SafeDict({"hello": "world"})
        assert d["hello"] == "world"

    def test_join_list_none(self) -> None:
        assert _join_list(None) == ""

    def test_join_list_empty_list(self) -> None:
        assert _join_list([]) == ""

    def test_join_list_basic(self) -> None:
        assert _join_list(["a", "b", "c"], sep=", ") == "a, b, c"

    def test_join_list_newline(self) -> None:
        result = _join_list(["line1", "line2"], sep="\n  ")
        assert "line1" in result
        assert "line2" in result

    def test_join_list_string_passthrough(self) -> None:
        assert _join_list("just a string") == "just a string"

    def test_build_hp_info_both_values(self) -> None:
        vision = {"player_hp_pct": 75, "enemy_hp_pct": 30}
        hp = _build_hp_info(vision)
        assert "75%" in hp
        assert "30%" in hp

    def test_build_hp_info_player_only(self) -> None:
        vision = {"player_hp_pct": 100}
        hp = _build_hp_info(vision)
        assert "100%" in hp
        assert "Enemy" not in hp

    def test_build_hp_info_none_values(self) -> None:
        vision: Dict[str, Any] = {}
        hp = _build_hp_info(vision)
        assert hp == ""

    def test_build_hp_info_fallback_hp_info_key(self) -> None:
        vision = {"hp_info": "PIKACHU hp: 20/35"}
        hp = _build_hp_info(vision)
        assert "PIKACHU" in hp

    def test_build_enemy_info_direct(self) -> None:
        vision = {"enemy_pokemon": "Zubat"}
        info = _build_enemy_info(vision)
        assert info == "Enemy: Zubat"

    def test_build_enemy_info_fallback(self) -> None:
        vision = {"enemy_info": "Wild ZUBAT appeared!"}
        info = _build_enemy_info(vision)
        assert info == "Wild ZUBAT appeared!"

    def test_build_enemy_info_none(self) -> None:
        vision: Dict[str, Any] = {}
        info = _build_enemy_info(vision)
        assert info == ""

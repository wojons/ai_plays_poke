...
class TestScreenshotProcessing:
    ...
    def test_vision_pipeline_latency(self) -> None:
        ...
        assert processing_time < 1.0, (
            f"Vision pipeline took {processing_time:.2f}s (>1s, warm)"
        )
    ...
    def test_location_detection_time(self) -> None:
        ...
        assert processing_time < 0.2, (
            f"Location detection took {processing_time:.2f}s (>200ms)"
        )
...
class TestMemoryUsage:
    ...
    def test_memory_within_limits(self) -> None:
        ...
        assert memory_increase < 100, (
            f"Memory increased by {memory_increase:.1f}MB (>100MB)"
        )
    ...
    def test_numpy_array_memory_efficient(self) -> None:
        ...
        assert memory_increase < 10, (
            f"Memory increased by {memory_increase:.1f}MB (>10MB)"
        )
...
class TestBenchmarks:
    ...
    def test_bench_ram_reader_observe(self, benchmark) -> None:
        ...
        emu.read_u16.side_effect = lambda addr: (
            _mem.get(addr, 0) | (_mem.get(addr + 1, 0) << 8)
        )
    ...
    def test_bench_pathfinding_find_path(self, benchmark) -> None:
        ...
        for x in range(size):
            for y in range(size):
                node = GraphNode(position=Position(x, y), tile_type=TileType.PASSABLE)
                graph.add_node(node)
                if x > 0:
                    graph.add_edge(
                        GraphEdge(

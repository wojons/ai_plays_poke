...
class TestTerrainEmojiTranslation:
    ...
    def test_known_terrain_emoji(self):
    ...
    def test_known_terrain_ascii(self):
    ...
    def test_unknown_terrain_ascii(self):
        """Unknown terrain chars are passed through as-is."""
        assert terrain_to_ascii("Z") == "Z"
        assert terrain_to_ascii("") == ""

    def test_all_terrain_keys_consistent(self):
...
class TestActorEmojiTranslation:
    ...
    def test_known_actor_emoji(self):
...
class TestFacingTranslation:
    ...
    def test_facing_emoji_north(self):
        assert facing_emoji("N") == "⬆️"

    def test_facing_emoji_south(self):
        assert facing_emoji("S") == "⬇️"

    def test_facing_emoji_east(self):
        assert facing_emoji("E") == "➡️"

    def test_facing_emoji_west(self):
        assert facing_emoji("W") == "⬅️"

    def test_facing_emoji_unknown(self):
        assert facing_emoji("X") == "❓"

    def test_facing_ascii_all(self):
    ...
    def test_facing_ascii_unknown(self):
...
class TestVisitedTranslation:
    ...
    def test_visited_emoji_known(self):
        assert visited_emoji("?") == "⬛"
        assert visited_emoji(".") == "⬜"
        assert visited_emoji("+") == "👣"
        assert visited_emoji("@") == "🎯"

    def test_visited_emoji_unknown(self):
        assert visited_emoji("X") == "⬛"  # fallback to never seen

    def test_visited_ascii_known(self):
        assert visited_ascii("?") == "?"
        assert visited_ascii(".") == "."
        assert visited_ascii("+") == "+"
        assert visited_ascii("@") == "@"

    def test_visited_ascii_unknown(self):
...
class TestWorldStateInitBlank:
    ...
    def test_creates_terrain_grid(self):
        ws = WorldState()
        ws.init_blank(5, 3)
...
class TestWorldStateGridAccess:
    ...
    def test_terrain_at_oob(self):
        ws = WorldState()
        ws.init_blank(5, 5)
        assert ws.terrain_at(999, 0) == "?"
        assert ws.terrain_at(-1, 2) == "?"

    def test_set_terrain_oob_silent(self):
        ws = WorldState()
        ws.init_blank(2, 2)
    ...
    def test_object_at_empty(self):
        ws = WorldState()
        ws.init_blank(5, 5)
        assert ws.object_at(2, 2) == " "

    def test_actor_at_found(self):
        ws = WorldState()
        ws.init_blank(10, 10)
    ...
    def test_actor_at_not_found(self):
        ws = WorldState()
        ws.init_blank(10, 10)
        assert ws.actor_at(5, 5) is None

    def test_edge_at_missing(self):
        ws = WorldState()
...
class TestComposedView:
    ...
    def test_basic_ascii_view(self):
        ws = WorldState()
        ws.init_blank(15, 11)
    ...
    def test_actor_rendered_in_ascii_view(self):
        ws = WorldState()
        ws.init_blank(5, 3)
        ws.viewport = Viewport(size=(5, 3), origin=(0, 0))
    ...
    def test_object_rendered_in_ascii_view(self):
        ws = WorldState()
        ws.init_blank(5, 3)
        ws.viewport = Viewport(size=(5, 3), origin=(0, 0))
    ...
    def test_player_has_priority_over_all(self):
...
class TestComposedViewCompact:
    ...
    def test_output_contains_nsew_edges(self):
        ws = WorldState()
        ws.init_blank(5, 5)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (2, 2)
    ...
    def test_visible_actors_in_viewport(self):
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (0, 0)
    ...
    def test_actors_outside_viewport_not_shown(self):
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (0, 0)
...
class TestParseObsPatchMovement:
    ...
    def test_moved_patch_from_yaml_string(self):
        yaml_str = """\
tick: 5
prev_tick: 4
movement:
...
class TestParseObsPatchBlocked:
    ...
    def test_blocked_patch_with_various_edge_keys(self):
        data = {
            "tick": 30,
...
class TestParseObsPatchCorrection:
    ...
    def test_correction_patch(self):
        data = {
            "tick": 50,
...
class TestParseObsPatchActorUpdates:
    ...
    def test_actor_updates_via_actors_key(self):
        data = {
            "tick": 6,
...
class TestParseObsPatchErrors:
    ...
    def test_visited_via_visited_add_key(self):
        data = {
            "tick": 2,
...
class TestObsPatchDataclassDefaults:
    ...
    def test_obs_patch_defaults(self):
        p = ObsPatch()
        assert p.tick == 0
    ...
    def test_strip_update_defaults(self):
        s = StripUpdate(edge="N")
        assert s.edge == "N"
        assert s.terrain == ""

    def test_resync_defaults(self):
        r = Resync()
        assert r.reason == ""
...
class TestValidatePatch:
    ...
    def test_valid_moved_patch(self):
        patch = ObsPatch(
            tick=10,
    ...
    def test_resync_skips_validation(self):
    ...
    def test_moved_more_than_one_tile(self):
        patch = ObsPatch(
            tick=5,
    ...
    def test_moved_viewport_scroll_no_strip(self):
        patch = ObsPatch(
            tick=5,
            movement=Movement(input="UP", result="moved", player_delta=[0, -1]),
    ...
    def test_moved_viewport_scroll_strip_no_terrain(self):
        patch = ObsPatch(
            tick=5,
            movement=Movement(input="UP", result="moved", player_delta=[0, -1]),
            viewport=ViewportDelta(new_edge="N"),
    ...
    def test_strip_terrain_too_long(self):
        patch = ObsPatch(
            tick=5,
            movement=Movement(input="UP", result="moved", player_delta=[0, -1]),
    ...
    def test_blocked_should_not_shift_viewport(self):
        patch = ObsPatch(
            tick=10,
    ...
    def test_blocked_should_record_edges(self):
        patch = ObsPatch(
            tick=10,
            movement=Movement(input="UP", result="blocked"),
            edges=[],
    ...
    def test_turned_only_delta_must_be_zero(self):
        patch = ObsPatch(
            tick=10,
    ...
    def test_warp_without_resync(self):
        patch = ObsPatch(
            tick=10,
    ...
    def test_battle_without_resync(self):
        patch = ObsPatch(
            tick=10,
    ...
    def test_dialog_without_resync(self):
        patch = ObsPatch(
            tick=10,
...
class TestMapIntegratorApplySuccess:
    ...
    def test_apply_moved_patch(self):
        ws = WorldState()
        ws.init_blank(20, 20)
    ...
    def test_apply_blocked_patch(self):
        ws = WorldState()
        ws.init_blank(20, 20)
        ws.player.pos = (5, 5)
        mi = MapIntegrator(world=ws)
    ...
    def test_apply_with_strip(self):
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.viewport = Viewport(size=(10, 10), origin=(0, 0))
        ws.player.pos = (5, 1)
    ...
    def test_apply_visited_marking(self):
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.player.pos = (0, 0)
        mi = MapIntegrator(world=ws)
    ...
    def test_apply_actor_update(self):
        ws = WorldState()
        ws.init_blank(20, 20)
        mi = MapIntegrator(world=ws)
    ...
    def test_apply_movement_mode_update(self):
        ws = WorldState()
        ws.init_blank(20, 20)
        ws.player.mode = "walk"
        mi = MapIntegrator(world=ws)
...
class TestMapIntegratorApplyRejection:
    ...
    def test_apply_rejects_invalid_patch(self):
        mi = MapIntegrator()
    ...
    def test_apply_rejects_bad_parse(self):
        mi = MapIntegrator()
        result = mi.apply("not valid at all: {{{")
        assert result is False
        assert mi.stats["patches_rejected"] == 1

    def test_apply_rejects_turned_only_with_delta(self):
        mi = MapIntegrator()
        ws = mi.world
        data = {
            "tick": 1,
...
class TestMapIntegratorComposeForController:
    ...
    def test_output_contains_map_and_mode(self):
        ws = WorldState()
        ws.init_blank(5, 5)
    ...
    def test_output_contains_player_pos(self):
        ws = WorldState()
        ws.init_blank(5, 5)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (3, 2)
...
class TestMapIntegratorStats:
    ...
    def test_stats_after_successful_apply(self):
        mi = MapIntegrator()
        mi.apply({
            "tick": 1,

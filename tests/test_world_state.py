...
class TestTerrainEmojiTranslation:
    ...
    def test_known_terrain_emoji(self):  # type: ignore
    ...
    def test_unknown_terrain_emoji(self):  # type: ignore
        """Unknown terrain chars fall back to ❓."""
        assert terrain_to_emoji("Z") == "❓"
        assert terrain_to_emoji("") == "❓"
        assert terrain_to_emoji("🐛") == "❓"

    def test_known_terrain_ascii(self):  # type: ignore
    ...
    def test_unknown_terrain_ascii(self):  # type: ignore
        """Unknown terrain chars are passed through as-is."""
        assert terrain_to_ascii("Z") == "Z"
        assert terrain_to_ascii("") == ""

    def test_all_terrain_keys_consistent(self):  # type: ignore
...
class TestActorEmojiTranslation:
    ...
    def test_known_actor_emoji(self):  # type: ignore
    ...
    def test_unknown_actor_emoji(self):  # type: ignore
        """Unknown actor kinds fall back to 👤."""
        assert actor_to_emoji("zzz") == "👤"
        assert actor_to_emoji("") == "👤"

    def test_known_actor_ascii(self):  # type: ignore
    ...
    def test_unknown_actor_ascii_fallback(self):  # type: ignore
        """Unknown actor kinds use first character as fallback."""
        assert actor_to_ascii("xyz") == "x"
        assert actor_to_ascii("") == "?"

    def test_all_actor_keys_consistent(self):  # type: ignore
...
class TestFacingTranslation:
    ...
    def test_facing_emoji_north(self):  # type: ignore
        assert facing_emoji("N") == "⬆️"

    def test_facing_emoji_south(self):  # type: ignore
        assert facing_emoji("S") == "⬇️"

    def test_facing_emoji_east(self):  # type: ignore
        assert facing_emoji("E") == "➡️"

    def test_facing_emoji_west(self):  # type: ignore
        assert facing_emoji("W") == "⬅️"

    def test_facing_emoji_unknown(self):  # type: ignore
        assert facing_emoji("X") == "❓"

    def test_facing_ascii_all(self):  # type: ignore
    ...
    def test_facing_ascii_unknown(self):  # type: ignore
...
class TestModeTranslation:
    ...
    def test_known_modes(self):  # type: ignore
    ...
    def test_unknown_mode(self):  # type: ignore
...
class TestEdgeEmoji:
    ...
    def test_known_outcomes(self):  # type: ignore
    ...
    def test_unknown_outcome(self):  # type: ignore
...
class TestVisitedTranslation:
    ...
    def test_visited_emoji_known(self):  # type: ignore
        assert visited_emoji("?") == "⬛"
        assert visited_emoji(".") == "⬜"
        assert visited_emoji("+") == "👣"
        assert visited_emoji("@") == "🎯"

    def test_visited_emoji_unknown(self):  # type: ignore
        assert visited_emoji("X") == "⬛"  # fallback to never seen

    def test_visited_ascii_known(self):  # type: ignore
        assert visited_ascii("?") == "?"
        assert visited_ascii(".") == "."
        assert visited_ascii("+") == "+"
        assert visited_ascii("@") == "@"

    def test_visited_ascii_unknown(self):  # type: ignore
...
class TestDescribeTile:
    ...
    def test_bare_terrain(self):  # type: ignore
        result = describe_tile(".")
        assert "⬜" in result

    def test_terrain_with_object(self):  # type: ignore
        result = describe_tile(".", obj="D")
        assert "⬜" in result
        assert "🚪" in result

    def test_terrain_with_actor(self):  # type: ignore
        result = describe_tile(".", actor_kind="n")
        assert "⬜" in result
        assert "👩‍⚕️" in result

    def test_terrain_object_and_actor(self):  # type: ignore
        result = describe_tile("~", obj="I", actor_kind="s")
        assert "🌊" in result   # water
        assert "💎" in result   # item
        assert "🏊" in result   # swimmer

    def test_space_object_skipped(self):  # type: ignore
...
class TestSymbolReference:
    ...
    def test_contains_terrain_symbols(self):  # type: ignore
        assert "g = 🌿 tall grass" in SYMBOL_REFERENCE
        assert "# = 🧱 wall" in SYMBOL_REFERENCE

    def test_contains_object_symbols(self):  # type: ignore
        assert "D = 🚪 door" in SYMBOL_REFERENCE
        assert "I = 💎 item" in SYMBOL_REFERENCE

    def test_contains_actor_kinds(self):  # type: ignore
...
class TestWorldStateInitBlank:
    ...
    def test_creates_terrain_grid(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 3)
    ...
    def test_creates_objects_grid(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(4, 4)
        assert len(ws.objects) == 4
        assert all(ch == " " for row in ws.objects for ch in row)

    def test_creates_visited_grid(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(3, 7)
...
class TestWorldStateGridAccess:
    ...
    def test_set_and_get_terrain(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.set_terrain(3, 5, "g")
        assert ws.terrain_at(3, 5) == "g"

    def test_terrain_at_oob(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 5)
        assert ws.terrain_at(999, 0) == "?"
        assert ws.terrain_at(-1, 2) == "?"

    def test_set_terrain_oob_silent(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(2, 2)
    ...
    def test_object_at_empty(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 5)
        assert ws.object_at(2, 2) == " "

    def test_actor_at_found(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
    ...
    def test_actor_at_not_found(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
        assert ws.actor_at(5, 5) is None

    def test_edge_at_missing(self):  # type: ignore
        ws = WorldState()
        assert ws.edge_at(0, 0, "N") is None

    def test_set_and_get_edge(self):  # type: ignore
        ws = WorldState()
        ws.tick = 42
...
class TestPlayerStateDefaults:
    ...
    def test_defaults(self):  # type: ignore
        p = PlayerState()
        assert p.pos == (0, 0)
        assert p.facing == "S"
...
class TestComposedView:
    ...
    def test_basic_ascii_view(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(15, 11)
    ...
    def test_emoji_view(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(3, 3)
    ...
    def test_actor_rendered_in_ascii_view(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 3)
        ws.viewport = Viewport(size=(5, 3), origin=(0, 0))
    ...
    def test_object_rendered_in_ascii_view(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 3)
        ws.viewport = Viewport(size=(5, 3), origin=(0, 0))
    ...
    def test_player_has_priority_over_all(self):  # type: ignore
...
class TestComposedViewCompact:
    ...
    def test_output_contains_header(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 5)
    ...
    def test_output_contains_nsew_edges(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 5)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (2, 2)
    ...
    def test_visible_actors_in_viewport(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (0, 0)
    ...
    def test_actors_outside_viewport_not_shown(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (0, 0)
...
class TestWorldStateSaveLoad:
    ...
    def test_save_and_load_roundtrip(self, tmp_path):  # type: ignore
        ws = WorldState()
        ws.tick = 42
        ws.map_id = "route1"
        ws.init_blank(4, 3)
    ...
    def test_load_empty_directory(self, tmp_path):  # type: ignore
        d = tmp_path / "empty"
        d.mkdir()
...
class TestParseObsPatchMovement:
    ...
    def test_moved_patch(self):  # type: ignore
        data = {
            "tick": 10,
            "prev_tick": 9,
            "movement": {
    ...
    def test_moved_patch_from_yaml_string(self):  # type: ignore
        yaml_str = """\
tick: 5
prev_tick: 4
movement:
...
"""
        ...
        assert patch.tick == 5
        assert patch.movement.result == "moved"  # type: ignore
        assert patch.movement.player_delta == [1, 0]  # type: ignore


class TestParseObsPatchBlocked:
    """parse_obs_patch with movement result: blocked."""

    def test_blocked_patch(self):  # type: ignore
        data = {
            "tick": 20,
            "movement": {
                "input": "UP",
        ...
        patch = parse_obs_patch(data)
        assert patch.movement.result == "blocked"  # type: ignore
        assert len(patch.edges) == 1
        assert patch.edges[0].outcome == "blocked"
        assert patch.edges[0].reason == "tree"

    def test_blocked_patch_with_various_edge_keys(self):  # type: ignore
        data = {
            "tick": 30,
...
class TestParseObsPatchResync:
    ...
    def test_resync_patch(self):  # type: ignore
        data = {
            "tick": 100,
            "resync": {
                "reason": "warp",
...
class TestParseObsPatchCorrection:
    ...
    def test_correction_patch(self):  # type: ignore
        data = {
            "tick": 50,
...
class TestParseObsPatchActorUpdates:
    ...
    def test_actor_updates(self):  # type: ignore
        data = {
            "tick": 5,
    ...
    def test_actor_updates_via_actors_key(self):  # type: ignore
        data = {
            "tick": 6,
...
class TestParseObsPatchErrors:
    ...
    def test_visited_as_dict(self):  # type: ignore
        data = {
            "tick": 1,
            "movement": {"input": "UP", "result": "moved"},
            "visited": {"add": [[2, 2]]},
    ...
    def test_visited_via_visited_add_key(self):  # type: ignore
        data = {
            "tick": 2,
...
class TestObsPatchDataclassDefaults:
    ...
    def test_obs_patch_defaults(self):  # type: ignore
        p = ObsPatch()
        assert p.tick == 0
    ...
    def test_movement_defaults(self):  # type: ignore
        m = Movement()
        assert m.result == "unknown"
        assert m.player_delta == [0, 0]

    def test_strip_update_defaults(self):  # type: ignore
        s = StripUpdate(edge="N")
        assert s.edge == "N"
        assert s.terrain == ""

    def test_resync_defaults(self):  # type: ignore
        r = Resync()
        assert r.reason == ""
...
class TestValidatePatch:
    ...
    def test_valid_moved_patch(self):  # type: ignore
        patch = ObsPatch(
            tick=10,
    ...
    def test_resync_skips_validation(self):  # type: ignore
    ...
    def test_moved_viewport_scroll_no_strip(self):  # type: ignore
        patch = ObsPatch(
            tick=5,
            movement=Movement(input="UP", result="moved", player_delta=[0, -1]),
    ...
    def test_moved_viewport_scroll_strip_no_terrain(self):  # type: ignore
        patch = ObsPatch(
            tick=5,
            movement=Movement(input="UP", result="moved", player_delta=[0, -1]),
            viewport=ViewportDelta(new_edge="N"),
    ...
    def test_strip_terrain_too_long(self):  # type: ignore
        patch = ObsPatch(
            tick=5,
            movement=Movement(input="UP", result="moved", player_delta=[0, -1]),
    ...
    def test_blocked_should_not_shift_viewport(self):  # type: ignore
        patch = ObsPatch(
            tick=10,
    ...
    def test_blocked_should_record_edges(self):  # type: ignore
        patch = ObsPatch(
            tick=10,
            movement=Movement(input="UP", result="blocked"),
            edges=[],
    ...
    def test_turned_only_delta_must_be_zero(self):  # type: ignore
        patch = ObsPatch(
            tick=10,
    ...
    def test_warp_without_resync(self):  # type: ignore
        patch = ObsPatch(
            tick=10,
    ...
    def test_battle_without_resync(self):  # type: ignore
        patch = ObsPatch(
            tick=10,
    ...
    def test_dialog_without_resync(self):  # type: ignore
        patch = ObsPatch(
            tick=10,
...
class TestMapIntegratorInit:
    ...
    def test_init_with_world_state(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(20, 20)
        mi = MapIntegrator(world=ws)
        assert mi.world is ws

    def test_init_blank_default(self):  # type: ignore
        mi = MapIntegrator()
        assert mi.world is not None
        assert len(mi.world.terrain) == 200
        assert len(mi.world.terrain[0]) == 200

    def test_init_from_directory(self, tmp_path):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
...
class TestMapIntegratorApplySuccess:
    ...
    def test_apply_moved_patch(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(20, 20)
    ...
    def test_apply_blocked_patch(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(20, 20)
        ws.player.pos = (5, 5)
        mi = MapIntegrator(world=ws)
    ...
    def test_apply_correction(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
    ...
    def test_apply_with_strip(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.viewport = Viewport(size=(10, 10), origin=(0, 0))
        ws.player.pos = (5, 1)
    ...
    def test_apply_resync(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(20, 20)
        ws.player.pos = (5, 5)
    ...
    def test_apply_visited_marking(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.player.pos = (0, 0)
        mi = MapIntegrator(world=ws)
    ...
    def test_apply_actor_update(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(20, 20)
        mi = MapIntegrator(world=ws)
    ...
    def test_apply_movement_mode_update(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(20, 20)
        ws.player.mode = "walk"
        mi = MapIntegrator(world=ws)
...
class TestMapIntegratorApplyRejection:
    ...
    def test_apply_rejects_invalid_patch(self):  # type: ignore
        mi = MapIntegrator()
    ...
    def test_apply_rejects_bad_parse(self):  # type: ignore
        mi = MapIntegrator()
        result = mi.apply("not valid at all: {{{")
        assert result is False
        assert mi.stats["patches_rejected"] == 1

    def test_apply_rejects_turned_only_with_delta(self):  # type: ignore
        mi = MapIntegrator()
        ws = mi.world
        data = {
            "tick": 1,
...
class TestMapIntegratorComposeForController:
    ...
    def test_output_is_string(self):  # type: ignore
        mi = MapIntegrator()
        output = mi.compose_for_controller()
        assert isinstance(output, str)

    def test_output_contains_map_and_mode(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 5)
    ...
    def test_output_contains_player_pos(self):  # type: ignore
        ws = WorldState()
        ws.init_blank(5, 5)
        ws.viewport = Viewport(size=(5, 5), origin=(0, 0))
        ws.player.pos = (3, 2)
...
class TestMapIntegratorStats:
    ...
    def test_initial_stats(self):  # type: ignore
        mi = MapIntegrator()
        s = mi.stats
    ...
    def test_stats_after_successful_apply(self):  # type: ignore
        mi = MapIntegrator()
        mi.apply({
            "tick": 1,
    ...
    def test_stats_rejection_rate(self):  # type: ignore
        mi = MapIntegrator()
    ...
    def test_recent_rejections_capped(self):  # type: ignore
        mi = MapIntegrator()
        for i in range(10):
...
class TestMapIntegratorSave:
    ...
    def test_save_persists(self, tmp_path):  # type: ignore
        ws = WorldState()
        ws.init_blank(10, 10)
        ws.set_terrain(5, 5, "#")
        ws.map_id = "test"

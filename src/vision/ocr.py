...
class OCREngine:
    ...
    def _save_font_database(self) -> None:
        ...
        data = {"font_templates": font_list, "special_cases": self.special_cases}
    ...
    def extract_text(self, image: np.ndarray, min_confidence: float = 0.7) -> OCRResult:
        """
        ...
        for line in lines:
            line_text, line_confidence, line_chars = self._recognize_line(
                line, min_confidence
            )
            all_text += line_text + "\n"
    ...
    def _template_match(self, char_region: np.ndarray, template: np.ndarray) -> float:

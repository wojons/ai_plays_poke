...
class LocationDetector:
    ...
    def detect_location(self, screenshot: np.ndarray) -> LocationResult:
        ...
        location_name, location_type, confidence = self._match_area(
            tile_patterns, features, pattern_hash
        )
    ...
    def _match_area(
        ...
        for area_key, area_info in self.area_database.items():
            ...
            if features.get("pokemon_center") and area_info.get("features", {}).get(
                "pokemon_center"
            ):
                score += 2.0
            if features.get("pokemart") and area_info.get("features", {}).get(
                "pokemart"
            ):
                score += 2.0
    ...
    def get_tile_collision(self, tile_type: str) -> str:
        return str(
            self.tile_classifications.get(tile_type, {}).get("collision", "unknown")
        )

    def is_tile_interactive(self, tile_type: str) -> bool:
        return bool(
            self.tile_classifications.get(tile_type, {}).get("interactive", False)
        )

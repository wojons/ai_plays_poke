...
class SpriteRecognizer:
    ...
    def _save_sprite_database(self) -> None:
        ...
        data = {
            "sprites": sprite_list,
            "common_pokemon": self.common_pokemon,
    ...
    def recognize_pokemon(
        ...
        if expected_types:
            candidates = [
                name
                for name in self.common_pokemon
                if any(t in self.pokemon_types.get(name, []) for t in expected_types)
            ] or self.common_pokemon
    ...
    def parse_hp_bar(self, hp_bar_image: np.ndarray) -> Optional[HPBarResult]:
        if hp_bar_image.size == 0:
            return None
        gray = (
            np.mean(hp_bar_image, axis=2).astype(np.uint8)
            if len(hp_bar_image.shape) == 3
            else hp_bar_image
        )
        h, w = gray.shape
    ...
    def detect_menu_cursor(self, menu_image: np.ndarray) -> Optional[MenuCursorResult]:
        ...
        return MenuCursorResult(
            position=cursor_y // option_height,
            option_count=4,
    ...
    def find_pokemon_sprites(
        ...
        if is_battle:
            h, w = image.shape[:2]
            enemy_region = image[
                int(h * 0.1) : int(h * 0.45), int(w * 0.45) : int(w * 0.9)
            ]
            player_region = image[
                int(h * 0.55) : int(h * 0.85), int(w * 0.05) : int(w * 0.5)
            ]

...
class DialogParser:
    ...
    def parse_dialog(
        ...
        primary_intent, secondary_intent, confidence = self._classify_intent(
            parsed_lines
        )
    ...
    def _classify_intent(
        ...
        primary_intent = (
            sorted_intents[0][0] if sorted_intents else DialogIntent.INFORMATION
        )
    ...
    def _detect_reward(self, lines: List[DialogLine]) -> Optional[Dict[str, Any]]:
        ...
        for line in lines:
            if line.intent == DialogIntent.REWARD or line.intent == DialogIntent.GIFT:
                return {
                    "type": "item",
                    "item": line.entities.get("items", ["Unknown"])[0]
                    if line.entities.get("items")
                    else "Unknown",
                    "source": line.speaker,
                }
...
class MenuNavigator:
    ...
    def detect_menu_type(self, screen_text: str) -> Optional[MenuType]:
        ...
        if "POKEMON" in text_upper and any(
            word in text_upper for word in ["BAG", "FIGHT", "RUN"]
        ):
...
class NPCInteraction:
    ...
    def _setup_default_npcs(self) -> None:
        ...
        default_npcs = [
            ...
            NPCInfo(
                name="MR FUJI", role="Shopkeeper", location="POKEMART", is_trainer=False
            ),
            NPCInfo(
                name="BROCK",
                role="Gym Leader",
    ...
    def _detect_battle_initiation(self, dialog_entry: DialogEntry) -> bool:
        ...
        if dialog_entry.primary_intent in [
            DialogIntent.BATTLE_CHALLENGE,
            DialogIntent.THREAT,
        ]:
            return True
    ...
    def get_interaction_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_time_ms": sum(r.time_spent_ms for r in self._interaction_history)
            / total
            if total > 0
            else 0.0,
        }
...
class DialogueManager:
    ...
    def process_dialog(
        ...
        self._system_stats["avg_processing_time_ms"] = (

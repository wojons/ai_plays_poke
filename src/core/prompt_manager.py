...
class PromptManager:
                ...
                for prompt_file in category_dir.glob("*.txt"):
                    try:
                        ...
                        if template:
                            ...
                            print(
                                f"✅ Loaded prompt: {template.category}/{template.name}"
                            )
    ...
    def get_relevant_prompts(
        ...
        for template in self.prompt_templates:
            ...
            elif game_state_type == "battle" and template.category in [
                "strategic",
                "battle",
            ]:
                relevant_prompts.append(template)
            elif game_state_type == "menu" and template.category in [
                "exploration",
                "strategic",
            ]:
                relevant_prompts.append(template)
            elif game_state_type == "overworld" and template.category in [
                "exploration",
                "strategic",
            ]:
                relevant_prompts.append(template)
            elif game_state_type == "dialog" and template.category in [
                "strategic",
                "battle",
            ]:
                relevant_prompts.append(template)
    ...
    def select_prompts_for_ai(
        ...
        for template in relevant_templates:
            if ai_preference == "balanced":
                ...
                if (
                    "strategic" in template.name.lower()
                    or template.category == "strategic"
                ):
                    selected_prompts.append(template.content)
            else:
    ...
    def get_prompt_analytics(self) -> Dict[str, Any]:
        ...
        for prompt_name, stats in self.prompt_usage_stats.items():
            analytics[prompt_name] = {
                ...
                "average_effectiveness": stats["effectiveness_sum"]
                / stats["usage_count"]

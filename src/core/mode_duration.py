...
class ModeClassifier:
    ...
    def classify_mode(
        ...
        sub_mode = self._determine_sub_mode(
            base_mode, visual_mode, text_context, current_state
        )
    ...
    def _get_text_context(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_shop": "shop" in dialog_text.lower() or "buy" in dialog_text.lower()
            if dialog_text
            else False,
...
class DurationProfileLearner:
    ...
    def update_profile(self, mode: str, sub_mode: str, duration: float) -> None:
        with self._lock:
            ...
            if profile.sample_count > self.min_samples:
                z_score = abs(duration - profile.mean_duration) / max(
                    profile.std_duration, 0.001
                )
            ...
            if profile.sample_count == 1:
                ...
                if profile.std_duration == 0:
                    ...
                    profile.std_duration = (
                        1 - self.alpha
                    ) * profile.std_duration + self.alpha * abs(duration - old_mean)
    ...
    def _update_percentiles(self, profile: ModeDurationProfile) -> ModeDurationProfile:
        ...
        profile.p75_duration = profile.mean_duration + 0.67 * max(
            profile.std_duration, 0.001
        )
        profile.p95_duration = profile.mean_duration + 1.645 * max(
            profile.std_duration, 0.001
        )
        profile.p99_duration = profile.mean_duration + 2.326 * max(
            profile.std_duration, 0.001
        )
        return profile
    ...
    def _update_trend(self, profile: ModeDurationProfile) -> ModeDurationProfile:
        ...
        recent_samples = [
            e.duration
            for e in DurationTracker().mode_history
            if e.exit_time > cutoff
            and e.mode == profile.mode
            and e.sub_mode == profile.sub_mode
        ]
    ...
    def get_thresholds(self, mode: str, sub_mode: str) -> Dict[str, float]:
        ...
        return {
            "warning": profile.p75_duration,
            "critical": profile.p95_duration,
    ...
    def _get_default_thresholds(self, mode: str, sub_mode: str) -> Dict[str, float]:
        defaults = {
            ...
            f"{GameMode.BATTLE.value}/WILD_NORMAL": {
                "warning": 300,
                "critical": 600,
                "emergency": 1200,
            },
            f"{GameMode.BATTLE.value}/WILD_HARD": {
                "warning": 600,
                "critical": 1200,
                "emergency": 2400,
            },
            f"{GameMode.BATTLE.value}/TRAINER": {
                "warning": 600,
                "critical": 1200,
                "emergency": 2400,
            },
            f"{GameMode.BATTLE.value}/GYM_LEADER": {
                "warning": 900,
                "critical": 1800,
                "emergency": 3600,
            },
            f"{GameMode.BATTLE.value}/ELITE_FOUR": {
                "warning": 1800,
                "critical": 3600,
                "emergency": 7200,
            },
            f"{GameMode.DIALOG.value}/NPC_SHORT": {
                "warning": 60,
                "critical": 180,
                "emergency": 300,
            },
            f"{GameMode.DIALOG.value}/NPC_LONG": {
                "warning": 600,
                "critical": 1200,
                "emergency": 2400,
            },
            f"{GameMode.DIALOG.value}/TUTORIAL": {
                "warning": 1800,
                "critical": 3600,
                "emergency": 7200,
            },
            f"{GameMode.DIALOG.value}/QUEST": {
                "warning": 900,
                "critical": 1800,
                "emergency": 3600,
            },
            f"{GameMode.DIALOG.value}/SHOP": {
                "warning": 300,
                "critical": 600,
                "emergency": 1200,
            },
            f"{GameMode.OVERWORLD.value}/NAVIGATION": {
                "warning": 300,
                "critical": 600,
                "emergency": 1200,
            },
            f"{GameMode.OVERWORLD.value}/INTERACTION": {
                "warning": 120,
                "critical": 300,
                "emergency": 600,
            },
            f"{GameMode.OVERWORLD.value}/PC": {
                "warning": 180,
                "critical": 300,
                "emergency": 600,
            },
            f"{GameMode.MENU.value}/PAUSE": {
                "warning": 120,
                "critical": 300,
                "emergency": 600,
            },
            f"{GameMode.MENU.value}/POKEMON": {
                "warning": 300,
                "critical": 600,
                "emergency": 1200,
            },
            f"{GameMode.MENU.value}/BAG": {
                "warning": 120,
                "critical": 300,
                "emergency": 600,
            },
            f"{GameMode.CUTSCENE.value}/INTRO": {
                "warning": 600,
                "critical": 1200,
                "emergency": 2400,
            },
            f"{GameMode.CUTSCENE.value}/EVOLUTION": {
                "warning": 180,
                "critical": 300,
                "emergency": 600,
            },
            f"{GameMode.CUTSCENE.value}/VICTORY": {
                "warning": 60,
                "critical": 120,
                "emergency": 180,
            },
...
class AnomalyResponseSelector:
    ...
    def select_response(self, anomalies: List[Anomaly]) -> ResponsePlan:
        ...
        for anomaly in sorted_anomalies:
            response = self.response_matrix.get(
                anomaly.type, {"actions": ["log_warning"], "confidence_impact": -5}
            )
            ...
            elif anomaly.severity == "MEDIUM" and highest_escalation not in [
                "HIGH",
                "EMERGENCY",
            ]:
...
class BreakoutManager:
    def __init__(self, emulator_controller: Optional[Any] = None):
        ...
        self.max_attempts = {
            ...
            BreakoutStrategy.STANDARD.value: 3,
            BreakoutStrategy.FORCE.value: 1,
        }
    ...
    def execute_breakout(
        ...
        for attempt in range(max_attempts):
            success, action = self._execute_strategy(
                strategy, mode, sub_mode, context or {}
            )
    ...
    def _states_equivalent(
        ...
        if not state1 or not state2:
            return False
        return state1.get("position") == state2.get("position") and state1.get(
            "hp"
        ) == state2.get("hp")
    ...
    def get_success_rate(self, strategy: str, mode: str) -> float:
        key = f"{strategy}/{mode}"
        relevant = [
            r for r in self.success_history if f"{r['strategy']}/{r['mode']}" == key
        ]
        if len(relevant) < 5:
            return 0.0
...
class BreakoutAnalytics:
    ...
    def record_breakout(self, result: BreakoutResult) -> None:
        ...
        key = (
            ...
            if result.mode
            else f"{result.strategy}/unknown"
        )
    ...
    def get_recommended_strategy(self, mode: str, sub_mode: str) -> BreakoutStrategy:
        strategies = [
            ...
            BreakoutStrategy.IMMEDIATE,
            BreakoutStrategy.FORCE,
        ]
...
class ModeDurationEscalation:
    def __init__(self, confidence_scorer: Optional[Any] = None):
        ...
        self.escalation_tiers: dict[EscalationTier, dict[str, Any]] = {
            ...
            EscalationTier.ENHANCED_MONITORING: {
                "confidence_floor": 60,
                "actions": ["increase_check_frequency", "log_verbose"],
                "check_interval": 5.0,
            },
            EscalationTier.PLAN_SIMPLIFICATION: {
                "confidence_floor": 40,
                "actions": ["simplify_strategy", "force_progress"],
                "check_interval": 2.0,
            },
            EscalationTier.EMERGENCY_PROTOCOL: {
                "confidence_floor": 20,
                "actions": ["activate_failsafe", "attempt_recovery"],
                "check_interval": 1.0,
            },
            EscalationTier.RESET_CONDITION: {
                "confidence_floor": 0,
    ...
    def _get_more_severe(
        self, tier1: EscalationTier, tier2: EscalationTier
    ) -> EscalationTier:
        tier_order = [
            EscalationTier.NONE,
            EscalationTier.ENHANCED_MONITORING,
...
class ModeDurationTrackingSystem:
    ...
    def update(
        ...
        for anomaly in anomalies:
            if anomaly.recommended_action.startswith("break_out"):
                strategy = self.analytics.get_recommended_strategy(
                    mode_classification.mode, mode_classification.sub_mode
                )
        ...
        return {
            ...
            "escalation_tier": new_tier.value,
            "response_actions": response.actions,
            "confidence_impact": response.confidence_impact,
        }

    def on_mode_exit(
        self, mode: str, sub_mode: str, reason: str, tick: int = 0
    ) -> None:
    ...
    def _is_mode_change(self, new_classification: ModeClassification) -> bool:
        ...
        return (
            current.mode != new_classification.mode
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            "current_mode": self.duration_tracker.current_mode.mode
            if self.duration_tracker.current_mode
            else None,
            "current_sub_mode": self.duration_tracker.current_mode.sub_mode
            if self.duration_tracker.current_mode
            else None,
            "current_duration": current_duration,
            "cumulative_session": current_cumulative,
            "escalation_tier": self.escalation.current_tier.value,
            "active_anomalies": len(
                self.anomaly_detector.detect_anomalies(
                    self.duration_tracker.current_mode.mode
                    if self.duration_tracker.current_mode
                    else "",
                    self.duration_tracker.current_mode.sub_mode
                    if self.duration_tracker.current_mode
                    else "",
                    current_duration,
                    current_cumulative,
                    0,
                    0,
                    self.duration_tracker.mode_sequence,
                )
            ),

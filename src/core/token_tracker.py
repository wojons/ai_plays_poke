    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                "avg_cost_per_call": round(self.total_cost / self.call_count, 6)
                if self.call_count > 0
                else 0.0,
                "avg_input_tokens": round(self.total_input_tokens / self.call_count, 1)
                if self.call_count > 0
                else 0,
                "avg_output_tokens": round(
                    self.total_output_tokens / self.call_count, 1
                )
                if self.call_count > 0
                else 0,
            }

...
| `--rom ROM` | `str` | `data/rom/Pokemon - Blue Version (USA, Europe) (SGB Enhanced).gb` | Path to the Gen-1 Game Boy ROM to boot (overrides the module default). The ROM is **not shipped with the repo** — `data/rom/` contains only a README; you must supply your own legally-obtained dump at the expected path. `--dry-run` reports the resolved ROM path and fails fast (exit 1) when the file is missing. |
...
Booting `data/boot.state` — a checkpoint saved from the Blue SGB ROM — with a non-Blue ROM loads garbage state with zero errors because PyBoy restores memory blindly. The runner detects the ROM-title mismatch and warns on both the real boot path and `--dry-run`; the warning names the mismatched title and points to `--boot-state skip` for non-Blue ROMs. The check is a no-op when the checkpoint is skipped, the ROM title is unreadable, or the ROM is Blue.
